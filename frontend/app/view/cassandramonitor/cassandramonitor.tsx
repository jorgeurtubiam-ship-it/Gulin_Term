...
export class CassandraMonitorViewModel {
    viewType = "cassandra-monitor";
    blockId: string;
    blockAtom: any;
    ...
    get viewComponent() { return CassandraMonitorView; }
}
function CassandraMonitorView({ model }: { model: CassandraMonitorViewModel }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e2e] text-white p-6 overflow-auto">
            <h2 className="text-2xl font-black mb-6 text-blue-400"><i className="fa fa-circle-notch mr-2"></i> Cassandra Gulin-Insights - {model.connectionName}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <MetricCard title="Estado" value={metrics?.service?.status || "OPEN"} icon="fa-power-off" color="text-blue-400" />
                <MetricCard title="Uso (GB)" value={metrics?.storage?.used_gb || "0"} icon="fa-hdd" color="text-blue-400" />
            </div>
