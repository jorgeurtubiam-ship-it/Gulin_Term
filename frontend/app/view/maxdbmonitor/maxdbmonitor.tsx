...
export class MaxdbMonitorViewModel {
    viewType = "maxdb-monitor";
    blockId: string;
    blockAtom: any;
    ...
    get viewComponent() { return MaxdbMonitorView; }
}
function MaxdbMonitorView({ model }: { model: MaxdbMonitorViewModel }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e2e] text-white p-6 overflow-auto">
            <h2 className="text-2xl font-black mb-6 text-teal-500"><i className="fa fa-server mr-2"></i> SAP MaxDB Gulin-Insights - {model.connectionName}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <MetricCard title="Estado" value={metrics?.service?.status || "OPEN"} icon="fa-power-off" color="text-teal-500" />
                <MetricCard title="Uso (GB)" value={metrics?.storage?.used_gb || "0"} icon="fa-hdd" color="text-teal-500" />
            </div>
