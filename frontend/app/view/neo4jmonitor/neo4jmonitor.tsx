...
export class Neo4jMonitorViewModel {
    viewType = "neo4j-monitor";
    blockId: string;
    blockAtom: any;
    ...
    get viewComponent() { return Neo4jMonitorView; }
}
function Neo4jMonitorView({ model }: { model: Neo4jMonitorViewModel }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e2e] text-white p-6 overflow-auto">
            <h2 className="text-2xl font-black mb-6 text-blue-600"><i className="fa fa-project-diagram mr-2"></i> Neo4j Gulin-Insights - {model.connectionName}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <MetricCard title="Estado" value={metrics?.service?.status || "OPEN"} icon="fa-power-off" color="text-blue-600" />
                <MetricCard title="Uso (GB)" value={metrics?.storage?.used_gb || "0"} icon="fa-hdd" color="text-blue-600" />
            </div>
