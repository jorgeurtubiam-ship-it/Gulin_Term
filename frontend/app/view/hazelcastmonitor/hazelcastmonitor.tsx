...
export class HazelcastMonitorViewModel {
    viewType = "hazelcast-monitor";
    blockId: string;
    blockAtom: any;
    ...
    get viewComponent() { return HazelcastMonitorView; }
}
function HazelcastMonitorView({ model }: { model: HazelcastMonitorViewModel }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e2e] text-white p-6 overflow-auto">
            <h2 className="text-2xl font-black mb-6 text-yellow-400"><i className="fa fa-bolt mr-2"></i> Hazelcast Gulin-Insights - {model.connectionName}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <MetricCard title="Estado" value={metrics?.service?.status || "OPEN"} icon="fa-power-off" color="text-yellow-400" />
                <MetricCard title="Uso (GB)" value={metrics?.storage?.used_gb || "0"} icon="fa-hdd" color="text-yellow-400" />
            </div>
