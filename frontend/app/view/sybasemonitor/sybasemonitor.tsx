...
export class SybaseMonitorViewModel {
    viewType = "sybase-monitor";
    blockId: string;
    blockAtom: any;
    ...
    get viewComponent() { return SybaseMonitorView; }
}
function SybaseMonitorView({ model }: { model: SybaseMonitorViewModel }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e2e] text-white p-6 overflow-auto">
            <h2 className="text-2xl font-black mb-6 text-green-500"><i className="fa fa-database mr-2"></i> Sybase ASE Gulin-Insights - {model.connectionName}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <MetricCard title="Estado" value={metrics?.service?.status || "OPEN"} icon="fa-power-off" color="text-green-500" />
                <MetricCard title="Uso (GB)" value={metrics?.storage?.used_gb || "0"} icon="fa-hdd" color="text-green-500" />
            </div>
