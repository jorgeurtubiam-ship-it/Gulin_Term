...
function DBConnectionsView({ model }: { model: DBConnectionsViewModel }) {
    ...
    const resultsMap = jotai.useAtomValue(model.resultsAtom);
    const results = activeTabId ? resultsMap[activeTabId] : null;
    ...
    const windowData = jotai.useAtomValue(GlobalModel.getInstance().windowDataAtom);
    const isStandalone = windowData?.meta?.bare === true;
                                    ...
                                    {editType === "oracle" ? (
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <input 
                                                ...
                                                placeholder={
                                                    editType === 'sqlite' ? "/ruta/absoluta/a/tu/archivo.db" :
                                                    editType === 'mysql' ? "user:pass@tcp(host:3306)/dbname" :
                                                    editType === 'mssql' ? "sqlserver://user:pass@host:1433?database=mydb" :
                                                    editType === 'mongodb' ? "mongodb://user:pass@host:27017/dbname" :
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <i className={`fa ${
                                                                ...
                                                                type === 'DATABASE' ? 'fa-database' :
                                                                type === 'USER' ? 'fa-user' :
                                                                type === 'ROLE' ? 'fa-users' :
                                                                type === 'SCHEMA' ? 'fa-sitemap' :
                                                                type === 'TYPE' ? 'fa-font' :
                                                                type === 'ASSEMBLY' ? 'fa-cogs' :
                                                                type === 'ENDPOINT' ? 'fa-plug' :
                                                                type === 'LINKED_SERVER' ? 'fa-server' :
                    ...
                    <Panel defaultSize={20} minSize={15} className="border-r border-zinc-800 flex flex-col bg-[#0c0c0e]">
                        <div className="p-4 border-b border-zinc-800/50 bg-[#09090b]/50">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <button 
                                    onClick={() => {
                                        const connType = dbs.find(d => d.name === selectedConn)?.type || "oracle";
                                        let monitorView = `${connType}-monitor`;
                                        // Backwards compatibility for existing widgets
                                        if (connType === "oracle") monitorView = "db-monitor";
                                        if (connType === "mssql") monitorView = "sqlserver-monitor";
                                        
                                        createBlock({ meta: { view: monitorView, connection: selectedConn } });
                                    }}
                                    className="bg-zinc-800/80 hover:bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded-md text-[9px] font-black uppercase tracking-widest flex items-center gap-2 transition-all border border-emerald-500/20 hover:border-emerald-500/40"
                                >
                        ...
                        <div className="p-4 border-t border-zinc-800/50 bg-[#09090b]/80 flex flex-col gap-2 shrink-0">
                            <button 
                                onClick={() => {
                                    const connType = dbs.find(d => d.name === selectedConn)?.type || "oracle";
                                    createBlock({ meta: { view: "sql-flow", sql: activeTab?.content || "", dialect: connType, connection: selectedConn } as any });
                                }}
                                className="bg-indigo-600/20 hover:bg-indigo-500/30 text-indigo-400 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all border border-indigo-500/20 hover:border-indigo-500/40 w-full"
                            >
                            ...
                            <button 
                                ...
                                disabled={executing}
                                className="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-purple-500/10 w-full"
                            >
                                {executing ? <i className="fa fa-spinner fa-spin"></i> : <i className="fa fa-play"></i>}
                                Ejecutar
                            </button>
                            <button 
                                onClick={() => model.sendToDashboard(selectedConn!)}
                                disabled={executing}
                                className="bg-emerald-600/20 hover:bg-emerald-500/30 text-emerald-400 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all border border-emerald-500/20 hover:border-emerald-500/40 w-full"
                            >
                        ...
                        <PanelGroup direction="vertical">
                            ...
                            <Panel defaultSize={60} minSize={30} className="flex flex-col bg-[#09090b]">
                                ...
                                <div className="flex items-center justify-between px-4 h-10 border-b border-zinc-800 bg-[#0c0c0e]/80">
                                    <div className="flex items-center h-full gap-1 overflow-x-auto no-scrollbar flex-grow">
                                        {tabs.map(tab => (
                                            <div
                                                key={tab.id}
                                                onClick={() => globalStore.set(model.activeTabIdAtom, tab.id)}
                                                title={tab.name}
                                                className={`flex items-center gap-2 px-4 h-full cursor-pointer text-[11px] font-mono border-b-2 transition-all group/tab flex-shrink-0 max-w-[200px] ${
                                                    activeTabId === tab.id ? 'bg-[#09090b] border-purple-500 text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300'
                                                }`}
                                            >
                                                <i className={`fa ${tab.type === 'sql' ? 'fa-file-code-o' : 'fa-table'} text-[10px] shrink-0`}></i>
                                                <span className="truncate">{tab.name}</span>
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); model.removeTab(tab.id); }}
                                                    className="ml-1 size-4 flex items-center justify-center rounded-full hover:bg-zinc-800 text-zinc-600 hover:text-red-400 opacity-0 group-hover/tab:opacity-100 transition-all shrink-0"
                                                >
                                                    <i className="fa fa-times text-[8px]"></i>
                                                </button>
                                            </div>
                                        ))}
                                        <button onClick={() => model.addTab()} className="px-3 text-zinc-600 hover:text-purple-400 shrink-0">
                                    ...
                                    {!isStandalone && (
                                        <div className="flex items-center gap-2">
                                            <button 
                                                onClick={() => {
                                                    if (block) {
                                                        WindowService.MoveBlockToNewWindow(model.tabModel.tabId, model.blockId);
                                                    }
                                                }}
                                                className="bg-zinc-800/80 hover:bg-purple-500/20 text-purple-400 px-3 py-1 rounded-md text-[9px] font-black uppercase tracking-widest flex items-center gap-2 transition-all border border-purple-500/20 hover:border-purple-500/40"
                                ...
                                <div className="flex-grow relative overflow-hidden">
                                    {activeTab?.type === 'sql' ? (
                                        <>
                                            <div className="absolute bottom-6 right-6 z-10 opacity-50 text-[10px] font-mono text-zinc-600 pointer-events-none">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {results && (
                                            <span className="text-[9px] text-zinc-600 font-mono">
                                                {results.rows?.length || 0} filas retornadas
                                            </span>
                                        )}
                                        {results && results.rows && results.rows.length > 0 && (
                                            <button 
                                                onClick={() => model.sendResultsToDashboard(results, (tabs.find(t => t.id === activeTabId)?.name) || 'Query')}
                                                className="bg-emerald-600/20 hover:bg-emerald-500/30 text-emerald-400 px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-all border border-emerald-500/20 hover:border-emerald-500/40"
                                            >
                                                <i className="fa fa-chart-pie"></i>
                                                Dashboard
                                            </button>
