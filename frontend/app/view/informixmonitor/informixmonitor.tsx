...
export class InformixMonitorViewModel {
    viewType = "informix-monitor";
    blockId: string;
    blockAtom: any;
    ...
    get viewComponent() { return InformixMonitorView; }
}
function InformixMonitorView({ model }: { model: InformixMonitorViewModel }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1e2e] text-white p-6 overflow-auto">
            <h2 className="text-2xl font-black mb-6 text-indigo-500"><i className="fa fa-hdd mr-2"></i> Informix Gulin-Insights - {model.connectionName}</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <MetricCard title="Estado" value={metrics?.service?.status || "OPEN"} icon="fa-power-off" color="text-indigo-500" />
                <MetricCard title="Uso (GB)" value={metrics?.storage?.used_gb || "0"} icon="fa-hdd" color="text-indigo-500" />
            </div>
