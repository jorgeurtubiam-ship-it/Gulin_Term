...
func MakeClient(appOpts AppOpts) *Client {
	...
	// Register brain routes automatically on client creation
	err := RegisterBrainRoutes(client)
	if err != nil {
		log.Printf("[brain] failed to register brain routes: %v\n", err)
	}
...
func (client *Client) Connect() error {
	...
	if jwtToken == "" {
		return fmt.Errorf("no %s env var set", wshutil.GulinJwtTokenVarName)
	}
	...
	client.ServerImpl = &GulinAppServerImpl{BlockId: client.RpcContext.BlockId, Client: client}
...
func (c *Client) CreateVDomContext(target *vdom.VDomTarget) error {
	...
	c.RpcClient.EventListener.On("blockclose", func(event *wps.GulinEvent) {
...
func makeNullVDom() *vdom.VDomElem {
	return &vdom.VDomElem{GulinId: uuid.New().String(), Tag: vdom.GulinNullTag}
}
...
func determineMimeType(option FileHandlerOption) (string, []byte) {
	...
	if option.FilePath != "" {
		filePath := gulinbase.ExpandHomeDirSafe(option.FilePath)
